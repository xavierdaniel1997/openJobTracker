                ...
                >
                    <div className="mb-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-xs text-text-secondary font-medium">{job.company}</span>
                            {job.platform && (
                                <span className={`px-1.5 py-0.5 rounded-[4px] text-[9px] uppercase tracking-wider font-bold border ${getPlatformStyle(job.platform)}`}>
                                    {job.platform}
                                </span>
                            )}
                        </div>
                            ...
                            {['applied', 'interview', 'offer', 'rejected'].map((s) => (
                                s !== job.status && (
                                    <button
                                        ...
                                        className="w-6 h-6 rounded-lg flex items-center justify-center bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/20 text-[10px] transition-all"
                                        title={`Move to ${s}`}
                                    >
                                        {s[0].toUpperCase()}
                                    </button>
